...
class UtilityReadingListScreen extends StatefulWidget {
  const UtilityReadingListScreen({
    super.key,
  ...
  @override
  State<UtilityReadingListScreen> createState() =>
      _UtilityReadingListScreenState();
}
...
class _UtilityReadingListScreenState extends State<UtilityReadingListScreen> {
  ...
  void _load() => _future =
      Future.wait([_service.getCurrentPrice(), _service.getReadings()]).then(
        (values) =>
            (values[0] as ServicePrice, values[1] as List<UtilityReading>),
      );
  ...
  Future<void> _openReadingForm([UtilityReading? reading]) async {
    final changed = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => UtilityReadingFormScreen(
  ...
  Future<void> _delete(UtilityReading reading) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          '${reading.roomNumber} tháng ${reading.month}/${reading.year}',
        ),
        actions: [
          TextButton(
        ...
        return RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      ...
                      label: Text(
                        widget.isManager ? 'Bảng giá' : 'Giá (chỉ xem)',
                      ),
                    ),
                  ),
                ],
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          Text(
                            'Điện: ${formatCurrency(price.electricPrice)}/kWh',
                          ),
              ...
              const SizedBox(height: 16),
              Text(
                '${readings.length} bản ghi gần nhất',
                ...
                (reading) => Padding(
                  ...
                  child: Card(
                    child: ListTile(
                      ...
                      title: Text(
                        '${reading.roomNumber} · ${reading.month.toString().padLeft(2, '0')}/${reading.year}',
                        style: const TextStyle(fontWeight: FontWeight.w600),
                      ),
                      subtitle: Text(
                        'Điện ${reading.oldElectric} → ${reading.newElectric} (${reading.electricUsage})\nNước ${reading.oldWater} → ${reading.newWater} (${reading.waterUsage})',
                      ),
                      isThreeLine: true,
