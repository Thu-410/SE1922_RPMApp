...
const activate = async (req, res, next) => { try { res.json({ success: true, message: 'Kích hoạt hợp đồng thành công', data: await service.activate(req.params.id) }); } catch (e) { next(e); } };
